...
class Detector():
    ...
    def __init__(self,
        ...
        if self.config.use_python_inference:
            self.executor, self.program, self.fecth_targets = load_executor(model_dir=model_dir, use_gpu=use_gpu)
        else:
            self.predictor = load_predictor(model_dir=model_dir,
                                            run_mode=run_mode,
...
def predict_image(image_path):
    results = detector.predict(image_path, args.threshold)
    return results
...
if __name__ == '__main__':
    ...
    parser.add_argument("--label_file",  type=str,   default='dataset/label_list.txt',           help="Path of label file.")
    parser.add_argument("--image_file",  type=str,   default='dataset/images/001422701011012.jpg', help="Path of image file.")
    parser.add_argument("--run_mode",    type=str,   default='fluid',  help="mode of running(fluid/trt_fp32/trt_fp16)")
    parser.add_argument("--use_gpu",     type=bool,  default=True,     help="Whether to predict with GPU.")
    parser.add_argument("--threshold",   type=float, default=0.01,      help="Threshold of score.")
    ...
    f = open('result.txt','a+')
    for im in images:
        img_path = 'dataset/test/images/' + im
        results = predict_image(img_path)
        img = cv2.imread(img_path)
        w, h, c = img.shape
        results_values = results["boxes"]
        for result in results_values:
            f.write(im[:-4] + ' '+ str(result[1]) + ' '+ str(int(result[2])) + ' ' 
            + str(int(result[3])) + ' ' + str(int(result[4])) + ' ' + str(int(result[5])) + '\n')
    f.close()
